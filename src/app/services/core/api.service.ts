...
export class ApiService {
  ...
  static apiUrl(apiName: string) {
    return environment.backend.apis[apiName].mode === 'gateway'
      ? `${environment.backend.apiGateway.baseUrl}${apiName}/`
      : `${environment.backend.apis[apiName].baseUrl}`;
  }

  constructor(private http: HttpClient) {
  }

  public getBuilder(builder): ApiService {

    this.url = environment.backend.apis[builder.api].mode === 'gateway'
      ? `${environment.backend.apiGateway.baseUrl}${builder.api}/${builder.path}${ApiService.obj2queryStr(builder.query)}`
      : `${environment.backend.apis[builder.api].baseUrl}${builder.path}${ApiService.obj2queryStr(builder.query)}`;
   ...
   /* if (builder.useAuthorization && this.authService.isAuthenticated()) {
      this.headers = this.headers.append('Authorization', this.authService.getAuthenticatedCredentials().authorization);
    }*/
  ...
  encrypt() {
    if (environment.production) {
      const params = this.url.replace(environment.backend.apiGateway.baseUrl, '');
      const crypt = CryptoJS.AES.encrypt(params, environment.undefined).toString();
      this.url = environment.backend.apiGateway.baseUrl + crypt;
    }
  }
  ...
  public postData(data: FormData) {
    ...
    /*console.log('[+] POST (data) ' + this.url);

    this.headers.delete('Content-Type');
    this.requestOpts = {headers: this.headers};

    return this.http.post(this.url, data, this.requestOpts)
      .toPromise()
      .then((res) => {
        return res.json() || {};
      })
